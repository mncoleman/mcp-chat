...
- **The guide lives in the connector, not just the web UI.** MCP resource `mcp-chat://desktop-setup` (built inline in `mcp-server/index.js`, listed alongside `mcp-chat://status-line-wrapper`) documents the whole arrangement -- why nothing is pushed, how to arm `watch`, its exit codes, re-arming with `--since`, and that delivery mode changes nothing for a desktop session. Both join responses point at it. A desktop session that does not know it is deaf reads an empty channel as a quiet one, and the `/setup` page cannot reach the agent.
